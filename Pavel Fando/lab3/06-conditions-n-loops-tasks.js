...
function isCreditCardNumber(ccn) {
    let data = ccn.toString();
    let sum = 0;
    let len = ccn.toString().length;
    for(let i = 0; i < len; i++) {
        let add = (data[i] - '0') * (2 - (i + len) % 2);
        add -= add > 9 ? 9 : 0;
        sum += add;
    }
    return sum % 10 === 0;
...
function getMatrixProduct(m1, m2) {
    let rows = m1.length, cols = m2[0].length, colsm1 = m1[0].length;
    let returnM = new Array(rows);
    for(let row = 0; row < rows; row++) {
        returnM[row] = new Array(cols).fill(0);
        for (let col = 0; col < cols; col++) {
            for(let i = 0; i < colsm1; i++) {
