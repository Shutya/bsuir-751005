...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var scalarMul = x1 * x2 + y1 * y2;
    var length1 = Math.sqrt(x1*x1 + y1*y1);
    var length2 = Math.sqrt(x2*x2 + y2*y2);
    var result = Math.acos(scalarMul / (length1 * length2));
    return result;
}
...
function getLastDigit(value) {
    ...
    var digitStr = valueStr[valueStr.length - 1];
...
function roundToPowerOfTen(num, pow) {
    var _pow = Math.pow(10, pow);
    var decimal = num / _pow
    var result = Math.round(decimal) * _pow;
    return result;
}
