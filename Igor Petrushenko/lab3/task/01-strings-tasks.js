...
function getRectangleString(width, height) {
    let middleLineStr = `│${" ".repeat(width-2)}│\n`;
    let topLineStr = `┌${'─'.repeat(width-2)}┐\n`;
    let bottomLineStr = `└${'─'.repeat(width-2)}┘\n`;
    let result = `${topLineStr}${middleLineStr.repeat(height-2)}${bottomLineStr}`;
    return result; 
}
...
function encodeToRot13(str) {
    function getLetter(letter) {        
        ...
        let alphabetLength = alphabetUpperCase.length;
        for (let i = 0; i < alphabetLength; i++) {
            if (letter == alphabetUpperCase.charAt(i)) return alphabetUpperCase.charAt((i + 13)%alphabetLength);
            if (letter == alphabetLowerCase.charAt(i)) return alphabetLowerCase.charAt((i + 13)%alphabetLength);
        }
