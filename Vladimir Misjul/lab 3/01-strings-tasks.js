...
function extractEmails(str) {
	var res = [];
	var indof=str.indexOf(';');
	var prevind = -1;
    while (prevind < str.length)
	{
		if (indof==-1)
		{
			res.push(str.substring(prevind+1, str.length));
			break;
		}
		res.push(str.substring(prevind+1, indof));
		prevind=indof;
		indof = str.indexOf(';', (indof+1));
	}
	return res;
}
 ...
 *
	...
	function encodeToRot13(str) {
		...
		{
			cifr = str.charCodeAt(i);
			if (cifr >= 'a'.charCodeAt() && cifr <= 'z'.charCodeAt())
			{
				cifr = (cifr + 13);
				if (cifr > 'z'.charCodeAt())
					cifr = 'a'.charCodeAt() + cifr - 'z'.charCodeAt()-1;
			}
			if (cifr >= 'A'.charCodeAt() && cifr <= 'Z'.charCodeAt())
			{
				cifr = (cifr + 13);
			...
			res = res.concat(String.fromCharCode(cifr)); 
		}
		return res;
	}
