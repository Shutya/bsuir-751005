...
function doRectanglesOverlap(rect1, rect2) {
    return (rect1.top<rect2.top+rect2.height && rect2.top<rect1.top+rect1.height || rect2.top<rect1.top+rect1.height && rect1.top<rect2.top+rect2.height) && (rect1.left<rect2.left+rect2.width && rect2.left<rect1.left+rect1.width || rect2.left<rect1.left+rect1.width && rect1.left<rect2.left+rect2.width);
}
...
function getCommonDirectoryPath(pathes) {
	...
	{
		len = Math.max(len, pathes[i].length);
	}
	for (let i = 0; i<len; i++)
	{	
		let eq=true;
		...
		if ((sym =='/') && eq)
			res = pathes[0].substring(0, i+1);
		if (!eq)
			return res;
		for (let j = 0; j<pathes.length; j++)
		{
			if (i+1>=pathes[j].length)
				return res;
		}
	}
	return res;
