...
function encodeToRot13(str) {
    var result = '';
    var vocablury = 'abcdefghijklmnopqrstuvwxyz';
    var isLower = false;
    ...
    for (var i = 0; i < str.length; i++){
        ...
        var temp = vocablury.indexOf(str[i].toLowerCase()); 

        if (temp != -1)
        {
            if (isLower)
                result += vocablury[(temp + 13) % 26];
            else    
                result += vocablury[(temp + 13) % 26].toUpperCase();
